...
import static warpspace.client.map.WarpSpaceMap.stars;

public class WarpMapDrawListener implements GameMapDrawListener {
	...
	public void galaxy_DrawQuads(GameMapDrawer drawer) {
		...
		if(WarpManager.isInWarp(sector)) {
			float r = WarpManager.getScale() / 2.0f;

			Vector3i realPos = WarpManager.getRealSpacePos(sector);
			drawCube(new Vector3f((realPos.x - 8.0f) * GameMapDrawer.sectorSize, (realPos.y - 8.0f) * GameMapDrawer.sectorSize, (realPos.z - 8.0f) * GameMapDrawer.sectorSize), r * GameMapDrawer.sectorSize, new Vector4f(0.0F, 1.0F, 1.0F, 0.25F));
