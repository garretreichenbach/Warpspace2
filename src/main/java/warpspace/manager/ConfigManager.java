...
public final class ConfigManager {
    ...
    private static final double LARGE_MAX = 1_000_000d;
    ...
    public static void initialize(WarpMain mod) {
        ...
        killswitchAstronautDrop = new SimpleConfigBool(serverConfig,
                "killswitch_astronaut_drop", true,
                "killswitch to disable automated astronaut drop. Disable if players crash with null pointers in warp.");
        killswitchSpeedDrop = new SimpleConfigBool(serverConfig,
                "killswitch_speeddrop", true,
                "killswitch to disable automated speed-drop from flying slowly in warp.");
        warpBeaconDisableOnHomebase = new SimpleConfigBool(serverConfig,
                "warp_beacon_disable_on_homebase", true,
                "Disable warp beacons on homebase stations.");

        droppointRandomOffset = new SimpleConfigDouble(serverConfig,
                "droppoint_random_offset_sectors", 2.25d,
                "Natural droppoints are randomly shifted up to this many sectors in each axis.");
        warpToRspRatio = new SimpleConfigDouble(serverConfig,
                "warp_to_rsp_ratio", 10d,
                "Travelling 1 sector in warpspace covers this many sectors in realspace. Droppoints are spaced this far apart.");
        secondsUntilSpeeddrop = new SimpleConfigDouble(serverConfig,
                "seconds_until_speeddrop", 30d,
                "Seconds spent flying too slowly in warpspace before speed-dropping.");
        minimumWarpSpeed = new SimpleConfigDouble(serverConfig,
                "minimum_warp_speed", 50d,
                "Required speed in m/s to remain stable in warpspace.");
        secondsWarpjumpDelay = new SimpleConfigDouble(serverConfig,
                "seconds_warpjump_delay", 9.5d,
                "Delay in seconds between activating the jumpdrive and the actual jump.");
        warpSpeedNoChamberMultiplier = new SimpleConfigDouble(serverConfig,
                "warp_speed_no_chamber_multiplier", 1d,
                "Max warpspace flight speed multiplier when no warp flight chamber is installed.");
        warpSpeedChamberLvl1Multiplier = new SimpleConfigDouble(serverConfig,
                "warp_speed_chamber_lvl_1_multiplier", 1.3d,
                "Max warpspace flight speed multiplier for warp flight chamber level 1.");
        warpSpeedChamberLvl2Multiplier = new SimpleConfigDouble(serverConfig,
                "warp_speed_chamber_lvl_2_multiplier", 1.6d,
                "Max warpspace flight speed multiplier for warp flight chamber level 2.");
        warpSpeedChamberLvl3Multiplier = new SimpleConfigDouble(serverConfig,
                "warp_speed_chamber_lvl_3_multiplier", 2d,
                "Max warpspace flight speed multiplier for warp flight chamber level 3.");
        warpBeaconChamberPercent = new SimpleConfigDouble(serverConfig,
                "warp_beacon_chamber_percent", 0.25d,
                "Fraction of the reactor tree consumed by the beacon chamber.");

        sfxEffectsEnable = new SimpleConfigBool(clientConfig,
                "sfx_effects_enable", true,
                "Enable warp sound effects.");
        vfxUseWarpShader = new SimpleConfigBool(clientConfig,
                "vfx_use_warp_shader", true,
                "Enable the colorful shader in warpspace. Disable if your GPU can't handle it.");

        sfxEffectsAddDb = new SimpleConfigDouble(clientConfig,
                "sfx_effects_loudness_add_dezibel", 0d,
                "Additional loudness for warp sound effects in decibels.");
        mapDrawDroppointsRange = new SimpleConfigDouble(clientConfig,
                "map_draw_droppoints_range", 1d,
                "Draw droppoints on the realspace map for warp sectors within this distance.");
    }
    ...
    public static double getDroppointRandomOffset() {
        return clamp(droppointRandomOffset, 0d, LARGE_MAX);
    }

    public static int getWarpToRspRatio() {
        return (int) clamp(warpToRspRatio, 2d, LARGE_MAX);
    }

    public static double getSecondsUntilSpeeddrop() {
        return clamp(secondsUntilSpeeddrop, 0d, LARGE_MAX);
    }

    public static double getMinimumWarpSpeed() {
        return clamp(minimumWarpSpeed, 0d, LARGE_MAX);
    }
    ...
    public static double getWarpSpeedNoChamberMultiplier() {
        return clamp(warpSpeedNoChamberMultiplier, 0d, 1000d);
    }

    public static double getWarpSpeedChamberLvl1Multiplier() {
        return clamp(warpSpeedChamberLvl1Multiplier, 0d, 1000d);
    }

    public static double getWarpSpeedChamberLvl2Multiplier() {
        return clamp(warpSpeedChamberLvl2Multiplier, 0d, 1000d);
    }

    public static double getWarpSpeedChamberLvl3Multiplier() {
        return clamp(warpSpeedChamberLvl3Multiplier, 0d, 1000d);
    }

    public static double getWarpBeaconChamberPercent() {
        return clamp(warpBeaconChamberPercent, 0d, 1d);
    }
    ...
    public static double getSfxEffectsAddDb() {
        return clamp(sfxEffectsAddDb, 0d, 20d);
    }

    public static int getMapDrawDroppointsRange() {
        return (int) clamp(mapDrawDroppointsRange, 0d, 10d);
    }
