...
public class WarpManager {
	 ...
	 */
	public static int offset = (int) (universeSize * (1 + 1.0f / getScale())) + 16 * 2; //offset in sectors
	...
	public static boolean isInWarp(Vector3i pos) {
		return pos != null && pos.y >= offset - (universeSize / getScale()) && pos.y <= offset + (universeSize / getScale());
	}
	...
	public static Vector3i getWarpSpacePos(Vector3i rspPos) {
		if(isInWarp(rspPos)) return rspPos;
		Vector3i warpPos;
	...
	public static Vector3i getRealSpacePos(Vector3i warpSpacePos) {
		if(!isInWarp(warpSpacePos)) return warpSpacePos;
		Vector3i realPos;
	...
	public static int getScale() {
		return ConfigManager.getWarpToRspRatio();
	}
}
